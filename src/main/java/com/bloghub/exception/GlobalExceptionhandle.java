...
public class GlobalExceptionhandle {
	...
	public ResponseEntity<ErrorResponse> handleMessagingException(MessagingException ex) {
		ex.printStackTrace();
		ErrorResponse re = new ErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(),
		...
		if (ex.getCause() instanceof InvalidFormatException invalidEx) {
			if (invalidEx.getTargetType().isEnum()) {
				message = "Invalid value '" + invalidEx.getValue() + "' for field '"
						+ invalidEx.getPath().get(0).getFieldName() + "'. Accepted values: "
						+ List.of(invalidEx.getTargetType().getEnumConstants());
			}
		}
